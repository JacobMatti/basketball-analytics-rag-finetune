...
# A larger context window, so long prompts aren't silently cut off.
NUM_CTX = 8192


def ollama_generate_full(model: str, prompt: str, json_schema: dict | None = None) -> dict:
    """Generate a completion and return Ollama's full response (text plus stats).
    ...
    :param json_schema: optional JSON schema. Ollama then constrains generation so the
        reply is always valid JSON matching the schema (its "format" option).
    :return: Ollama's JSON body; "response" is the text, "prompt_eval_count" is how
        many prompt tokens the model actually read, "eval_count" how many it wrote,
        and "total_duration" the time taken in nanoseconds
    """
...
def ollama_generate(model: str, prompt: str) -> str:
    """Generate a completion and return only its text (see ollama_generate_full)."""
    return ollama_generate_full(model, prompt)["response"]
